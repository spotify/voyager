...
std::optional<std::size_t> parsePositiveInteger(const std::string &value) {
  try {
    ...
    const std::size_t firstCharacter = value.find_first_not_of(" \t\n\r");
...
std::optional<std::size_t> cpuCountForQuota(std::size_t quota,
                                            std::size_t period) {
  if (quota == 0 || period == 0)
    return std::nullopt;
...
bool hasOctalEscapeAt(const std::string &path, std::size_t offset) {
...
bool isRequestedCgroupMount(const std::vector<std::string> &fields,
                            std::size_t separator, bool unified) {
...
std::optional<CgroupMount> parseCgroupMount(const std::string &line,
                                            bool unified) {
  ...
  const std::size_t separatorIndex = separator - fields.begin();
...
std::optional<CgroupMount> findCpuCgroupMount(const std::string &mountInfo,
...
std::optional<std::string> findCpuCgroupPath(const std::string &cgroupContents,
                                             bool unified) {
...
std::optional<std::size_t> quotaAt(const std::string &directory, bool unified,
...
std::optional<std::size_t> tightestQuota(const std::string &directory,
...
std::optional<std::size_t> affinityCpuCount() {
#ifdef __linux__
...
availableCpuCountFromLimits(std::size_t hardwareCount,
  ...
  std::size_t available = std::max<std::size_t>(1, hardwareCount);
  if (affinityCount)
    available = std::min(available, *affinityCount);
  if (quotaCount)
    available = std::min(available, *quotaCount);
  return std::max<std::size_t>(1, available);
}

} // namespace detail

std::size_t availableCpuCount() {
  const std::size_t hardwareCount = std::thread::hardware_concurrency();
  const auto affinityCount = detail::affinityCpuCount();
#ifdef __linux__
  const auto quotaCount = detail::cgroupCpuCount(detail::readFile);
#else
  const std::optional<std::size_t> quotaCount = std::nullopt;
#endif
  return detail::availableCpuCountFromLimits(hardwareCount, affinityCount,
                                             quotaCount);
}
