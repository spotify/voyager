"""
Cribbed from Pedalboard as of 2023/06/14.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate type stub files (.pyi) and Sphinx documentation for Voyager."
    )
    parser.add_argument(
        "--docs-output-dir",
        default=os.path.join(os.path.dirname(__file__), "..", "..", "docs", "python"),
        help="Output directory for documentation HTML files.",
    )
    parser.add_argument(
        "--docs-input-dir",
    ...
    if not args.skip_regenerating_type_hints:
        with isolated_imports({"voyager"}):
            ...
            # Generate .pyi stubs files from Voyager's native (Pybind11) source.
            ...
            postprocess_type_hints_main(
                ["voyager", "voyager"] + (["--check"] if args.check else [])
            )
    ...
    print("Importing numpy to ensure a successful Voyager stub import...")
    import numpy  # noqa

    print("Importing .pyi files for our native modules...")
    for modname in ["voyager"]:
        import_stub(".", modname)
