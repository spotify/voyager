...
def test_load_from_subprocess(
    ...
    reloaded = voyager.Index.load_from_subprocess(
        f"cat {str(output_file)}",
        space=space,
        num_dimensions=num_dimensions,
        storage_data_type=storage_data_type,
    )

    assert index.as_bytes() == reloaded.as_bytes()
...
def test_load_from_subprocess_propagates_errors(
    storage_data_type: voyager.StorageDataType,
):
    with pytest.raises(RuntimeError):
        voyager.Index.load_from_subprocess(
            "false",
