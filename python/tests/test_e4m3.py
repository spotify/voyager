...
def normalize_e4m3(vec: np.ndarray) -> np.ndarray:
    """
    Given an input vector, normalize it to have a magnitude of 1.
    If E4M3 conversion would result in a magnitude greater than 1, scale the vector down
    so that its magnitude is at most 1.
    """
    vec = np.array(vec).astype(np.float32)

    error_correction_factor = 0
    while True:
        magnitude = np.sqrt(np.sum(np.power(vec, 2))) + 1e-30 + error_correction_factor
        e4m3_candidate = np.array([float(E4M3T(x)) for x in vec / magnitude])
        resulting_magnitude = np.sqrt(np.sum(np.power(e4m3_candidate, 2)))
        if resulting_magnitude <= 1:
            break
        error_correction_factor += 0.0009765625

    return np.array(e4m3_candidate)


def test_cosine():
    REAL_WORLD_VECTOR = np.array(
        [
            -0.28728199005126953,
            -0.4670010209083557,
            0.2676819860935211,
    ...
    np.testing.assert_allclose(
        ...
        err_msg=(
            ...
            f" {REAL_WORLD_VECTOR[mismatch_indices]}"
        ),
    )
...
def test_normalization(input, expected):
    magnitude = np.sqrt(np.sum(np.power(E4M3T.normalize(input), 2)))
